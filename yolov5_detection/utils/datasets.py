...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #     f = []
        #
        #     for l in path if isinstance(path, list) else [path]:
        #         dirs = os.listdir(l)
        #         for dir in dirs:
        #             z = []
        #             if os.path.isdir(os.path.join(l, dir)):
        #                 files = os.listdir(os.path.join(l, dir))
        #                 for file in files:
        #                     if file.split('.')[-1] == 'png':
        #                         z.append(file.split('.')[0])
        #                 z = sorted(z, key=lambda x: int(x), reverse=False)
        #                 for i in z:
        #                     f.append(l + '/' + dir + '/' + i + '.png')
        #     self.img_files = f
        #
        # except Exception as e:
        #     raise Exception(f'{prefix}Error loading data from {path}: {e}\nSee {help_url}')
