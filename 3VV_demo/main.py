...
def getModel(args):
    ...
    if args.arch == 'deeplabv3':
        from method_utils.deeplabv3 import DeepLabV3
        model = DeepLabV3(1, '/data/ylf/3VV_demo/log/deeplabv3/').to(device)
    return model
...
def getDataset(args):
    ...
    if args.dataset == 'MyDataset':  # E:\代码\new\u_net_liver-master\data\liver\val
        train_dataset = MyDataset_CE(r"train", transform=x_transforms, target_transform=y_transforms)
        train_dataloaders = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True)
        val_dataset = MyDataset_CE(r"val", transform=x_transforms, target_transform=y_transforms)
        val_dataloaders = DataLoader(val_dataset, batch_size=1, shuffle=True)
        test_dataset = MyDataset_CE(r"test", transform=x_transforms, target_transform=y_transforms)
...
def train(model, criterion1, criterion2, optimizer, train_dataloader, val_dataloader, args):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch > 15 and scheduler.get_last_lr()[0] > 1e-5:
            scheduler.step()
...
def val(model, best_iou, val_dataloaders, val_loss_list):
    ...
    with torch.no_grad():
        ...
        if aver_iou > best_iou:
            ...
            torch.save(model.state_dict(), r'.../3VV_demo/saved_model/full_size/' + str(args.arch) + '_' + str(
...
def test(val_dataloaders, save_predict=False):
    ...
    if save_predict == True:
        dir = r'.../3VV_demo/result/full_size/' + str(args.batch_size) + '/'
        if not os.path.exists(dir):
            os.makedirs(dir)
        else:
            print('dir already exist!')

    model.load_state_dict(torch.load(r'.../3VV_demo/saved_model/full_size/' + str(args.arch) + '_' + str(
    ...
    with torch.no_grad():
        ...
        for pic, _, pic_path, mask_path, ce_label in val_dataloaders:
            ...
            if save_predict == True:
                ...
                cv2.imwrite(dir + mask_path[0].split('/')[-1], pred)
...
if __name__ == "__main__":
    ...
    if 'test' in args.action:
        test(test_dataloaders, save_predict=False)
