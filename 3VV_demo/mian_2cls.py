...
def getModel(args):
    ...
    if args.arch == 'deeplabv3':
        from method_utils.deeplabv3 import DeepLabV3_2cls
        model = DeepLabV3_2cls(1, '.../3VV_demo/log/deeplabv3/').to(device)
    return model
...
def getDataset(args):
    ...
    if args.dataset == 'MyDataset':  # E:\代码\new\u_net_liver-master\data\liver\val
        train_dataset = MyDataset_2cls(r"train", transform=x_transforms, target_transform=y_transforms)
        train_dataloaders = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, drop_last=True)
        val_dataset = MyDataset_2cls(r"val", transform=x_transforms, target_transform=y_transforms)
        val_dataloaders = DataLoader(val_dataset, batch_size=1, shuffle=True)
        test_dataset = MyDataset_2cls(r"test", transform=x_transforms, target_transform=y_transforms)
    ...
    for epoch in range(num_epochs):
        ...
        for x, y, _, mask in train_dataloader:
            ...
            if args.deepsupervision:
                ...
                output = model(inputs)
                loss_1 = criterion1(torch.sigmoid(output), labels)
                loss_2 = criterion2(torch.sigmoid(output), labels)
...
def val(model, best_iou, val_dataloaders, val_loss_list):
    ...
    with torch.no_grad():
        ...
        for x, _, pic, mask in val_dataloaders:
            ...
            hd = get_hd_2cls(mask[0], img_y)
            hd_total += hd

            iou, dice = get_iou_2cls(mask[0], img_y)
        ...
        if aver_iou > best_iou:
            ...
            torch.save(model.state_dict(), r'.../3VV_demo/saved_model/2cls_seg/' + str(args.arch) + '_' + str(
...
def test(val_dataloaders, save_predict=False):
    ...
    if save_predict == True:
        dir = r'.../3VV_demo/result/2cls_seg/' + str(args.batch_size) + '/'
        if not os.path.exists(dir):
            os.makedirs(dir)
        else:
            print('dir already exist!')

    model.load_state_dict(torch.load(r'.../3VV_demo/saved_model/2cls_seg/' + str(args.arch) + '_' + str(
    ...
    with torch.no_grad():
        ...
        for pic, _, pic_path, mask_path in val_dataloaders:
            ...
            iou, dice = get_iou_2cls(mask_path[0], predict)
            hd = get_hd_2cls(mask_path[0], predict)

            hd_total += hd

            miou_total += iou[0] # 获取当前预测图的miou，并加到总miou中
            dice_total += dice[0]

            if save_predict == True:
                pred = recover_image_size_1cls(predict, mask_path[0])

                cv2.imwrite(dir + mask_path[0].split('/')[-1], pred*255)

            print('iou={},dice={}'.format(iou, dice))
            if i < num: i += 1  # 处理验证集下一张图

        print('Miou=%f,aver_hd=%f,M_dice=%f' % (miou_total / num, hd_total / num, dice_total / num))
